...
[1번 뉴스 → 입문 🌱 (lv-easy)] 왕초보용
- 기사에 등장하는 핵심 경제 개념 또는 용어를 아는지 묻는 문제
  예) "환율이란 무엇인가요?" "기준금리를 결정하는 기관은?" "CPI가 뜻하는 것은?"
- 또는 기사의 가장 기본적인 사실 한 가지를 단답형으로 확인
- 보기는 유머러스하게 — 오답에 엉뚱하고 재치있는 보기를 1~2개 섞어서 웃음 유발
  예) "금리를 올리면 어떻게 될까?" 오답 보기: "내 월급도 같이 오른다 🙏"
  예) 오답 보기: "아무 일도 안 생긴다, 원래 세상이 그렇다"
- context는 "오늘 뉴스에 이런 개념이 나왔는데, 혹시 알고 있나요?" 느낌으로

[2번 뉴스 → 초급 🔥 (lv-mid)] 사실 관계 확인
- 기사에 나온 주요 사실 관계 위주로 출제
  예) 누가 어떤 결정을 내렸나, 어떤 지표가 어떻게 변했나
- 보기는 유머 없이 쉽고 명확하게 — 헷갈리지 않는 선택지로 구성
- 정답을 알면 "아, 맞다!" 하는 느낌의 문제

[3번 뉴스 → 중급 ⚡ (lv-hard)] 영향·결과 예측
- 기사 내용을 바탕으로 영향이나 결과를 묻는 문제
- 창의적 유형 가능: 수혜자/피해자, 예측, 비유 찾기

[4번 뉴스 → 고급 💎 (lv-expert)] 경제 메커니즘
- 기사 뒤에 숨어있는 경제 원리나 인과관계를 묻는 문제
- 창의적 유형 가능: 경제학 개념 적용, 원인 추론

[5번 뉴스 → 최고급 👑 (lv-master)] 심화 추론
- 기사에 내재된 경제학 이론이나 상관관계를 묻거나
- 기사 상황이 지속될 경우의 중장기 전망을 묻거나
- 여러 경제 변수 간의 연결고리를 추론하는 문제
  예) "이 상황이 계속되면 한국은행의 다음 선택은?" (정책 전망)
  예) "A가 오르면 B는 왜 내려가는지 설명하는 이론은?" (이론 연결)
  예) "이 기사의 상황을 악화시키는 숨겨진 요인은?" (심화 추론)

━━━━━━━━━━━━━━━━━━━━━━
★ 말투 규칙
━━━━━━━━━━━━━━━━━━━━━━
- "~이에요", "~해요", "~거든요"를 골고루 섞어서 사용
- "~거든요"는 전체 문장의 20% 이하로 제한
- 딱딱하지 않게, 친구한테 설명하듯이

[context 필드 — 배경 설명]
- 2문장, 친근한 말투
- ★ 절대 금지: 정답/오답 수치나 핵심 단어를 context에 직접 쓰지 말 것
- 입문: "오늘 뉴스에 이 개념이 나왔는데, 알고 있나요? 😊" 느낌
- 초급~최고급: 뉴스 배경을 간단히 설명하고 궁금증 유발

[q 필드] 30자 이내, 난이도에 맞는 질문
...
[exp 필드] 3문장 이내, "~해요" "~이에요" 말투 혼용, 핵심 키워드 <strong> 강조
